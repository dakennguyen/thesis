pragma solidity ^0.5.0;

contract Dns {
    mapping(string => Validator) validators;
    mapping(string => PublicKey) publicKeys;
    address private owner;

    struct Validator {
        string name;
        string apiLink;
    }

    struct PublicKey {
        bytes modulus;
        bytes exponent;
    }

    constructor() public {
        owner = msg.sender;

        validators['age'] = Validator('egov', 'https://validators-thesis.azurewebsites.net/api/sign/egov');
        validators['name'] = Validator('egov', 'https://validators-thesis.azurewebsites.net/api/sign/egov');
        validators['uni'] = Validator('ministry', 'https://validators-thesis.azurewebsites.net/api/sign/ministry');

        publicKeys['egov'] = PublicKey(hex"", hex"010001");
        publicKeys['ministry'] = PublicKey(hex"", hex"010001");
    }

    function setValidator(string memory attributeType, string memory name, string memory apiLink) public onlyOwner {
        validators[attributeType] = Validator(name, apiLink);
    }

    function getValidator(string memory attributeType) public view returns(string memory) {
        return validators[attributeType].apiLink;
    }

    function setPublicKey(string memory name, bytes memory modulus, bytes memory exponent) public onlyOwner {
        publicKeys[name] = PublicKey(modulus, exponent);
    }

    function getPublicKey(string memory attributeType) public view returns(bytes memory, bytes memory) {
        string memory name = validators[attributeType].name;
        return (publicKeys[name].modulus, publicKeys[name].exponent);
    }

    modifier onlyOwner {
        require(msg.sender == owner, "Only owner can call this function.");
        _;
    }
}
