...
namespace validators.Controllers
{
    ...
    public class SignController : ControllerBase
    {
        ...
        [HttpGet("ministry")]
        public IActionResult Ministry([FromQuery]string data)
        {
            ...
            Result tmp = this.Sign(data, pkeyXml);
            return Ok(tmp);
        }

        [HttpGet("egov")]
        public IActionResult Egov([FromQuery]string data)
        {
        ...
        private Result Sign(string data, string privateKey)
        {
            Result result = new Result();
            byte[] bytes = Encoding.ASCII.GetBytes(data.PadLeft(64, '\0'));
            using(SHA256 sha256Hash = SHA256.Create())
            {
                byte[] hex = sha256Hash.ComputeHash(bytes);
                result.Hash = BitConverter.ToString(hex).Replace("-", "").ToLower();
            }
            using(RSACryptoServiceProvider rsa = new RSACryptoServiceProvider(keySize))
            {
                rsa.FromXmlString(privateKey);
                byte[] hex = rsa.SignData(bytes, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);
                result.Signature = BitConverter.ToString(hex).Replace("-", "").ToLower();
            }
            return result;
        }
    }
